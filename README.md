...
There are several technical limitations to the current model. Firstly, the simple, subjective nature of the heuristic approach introduces bias to the model. This tunable expert system is designed to capture my interpretation of "compatability", but my personal opinion on what makes people compatible should not the the sole quantifier. This type of model was chosen in favour of a more objective black-box model for full transcapency, and explainablility: a deliberate choice to meet the project requirements of clear thinking a justifications. ** what technical limitations are there in terms of problems my model overlooks? The project also relies heavily on a single API call. An API failure, network timeut or malformed

LLM reliability: project hinges on since API call. Tradeoff is performance and cost efficiency, avoiding complexity and chained API calls. mitigated using try-except. Current implementation runs entire pipelin wthin a single requrest: not suitable for high traffic. Design choice for simplicity and rapid devleopment appropriate for 48h task. Focus was not on scalable production-grade infrastructure.
