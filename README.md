...
There are several technical limitations to the current model, due to the pragmatic shortcuts making in light of the tight time frame. Firstly, the simple, subjective nature of the heuristic approach introduces bias to the model. This tunable expert system is designed to capture my interpretation of "compatability", but my personal opinion on what makes people compatible should not the the sole quantifier. This type of model was chosen in favour of a more objective black-box model for full transcapency, and explainablility: a deliberate choice to meet the project requirements of clear thinking a justifications. The project also relies heavily on a single API call. An API failure, network timeut or malformed JSON reponse would break the entire analysis, but it is faster and cheaper to run a single multi-task prompt than 3 or 4 sequential API calls. This risk of the multi-task method was mitigated using a try-except block which prevents crashing. The current model implementation also runs Whisper and the LLM analysis synchronously, which is not suitable for high traffic. This basic design choice was made for simplicity and rapid development appropirate for a 48h task. The focus was on simplicity, not scalable production-grade infrastructure. There are also several key solutions to these limitations, and methods of improving the model to a more nuanced understanding of compatability, covering in the "Future Steps" section.


## Future Steps
With more time to refine the model, there are many changes I would look to implement. Firstly, speaker diarisation would be used to segment the transcripy by speaker. This would unlock more grandular analysis, and provide individual engagement scores. The topic interest vector is the minimum of the interest of the two users, quantifying how interested the least interested person is the cinversation. Speaker diarisation would allow a similar thing to be done for the engagement score. **my attempt. The heuristic subjectivity could also be vastly improved upon. With more time, a systematic search over the weight parameters could have been done to find the set which most accurately aligns with human observed judgement. An indepth discussion with researchers in the field of compatability would also help refine the metrics used, and a data-driven ML solution based on their expertese could certainly be created. The pipeline could also be refactored to reduce the synchronicity of the model, making it more scalable for higer traffic environments. I would also like to encorporate analysis directly from the audio sources. By looking at the tone, pitch and tempo of peoples voice, assumptions about their compatability with others could be made. Incorporating biometrics would also be a powerful feature, since people's physiological signals can start to align when with someone compatible.
